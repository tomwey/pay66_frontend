        ...
        <el-table-column label="操作" fixed="right" align="center" width="240">
          <template slot-scope="scope">
            ...
            <el-button
              size="mini"
              type="warning"
              v-if="scope.row.opened && $hasPermission(119,1010)"
              @click="openItem(scope.row, false);"
            >禁用</el-button>
            <el-button
              size="mini"
              type="success"
              v-if="!scope.row.opened && $hasPermission(119,1009)"
...
export default {
  ...
  methods: {
    loadData(conds) {
      ...
      this.$get("admin/common/tag/list", params, res => {
        // console.log(res);
        this.loading = false;
        if (res.code == 0) {
          this.list = res.data;
    ...
    deleteItem(item) {
      this.$confirm("删除后不能恢复，您确定吗？", "删除提示", {
        ...
        .then(() => {
          this.loading = true;
          this.$post("admin/common/tag/delete", { ids: item.id }, res => {
            this.loading = false;
            if (res.code == 0) {
              this.$message({
                type: "success",
    ...
    openItem(item, flag) {
      this.loading = true;
      this.$post(
        "admin/common/tag/open_or_close",
