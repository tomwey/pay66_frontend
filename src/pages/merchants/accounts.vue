...
export default {
  ...
  methods: {
    ...
    loadData(conds = []) {
      ...
      this.$get("admin/common/merch_account/list", params, res => {
        // console.log(res);
        this.loading = false;
        if (res.code == 0) {
          this.list = res.data;
    ...
    loadRoles() {
      this.loading = true;
      this.$get("admin/configs/merch_roles", null, res => {
        // console.log(res);
        this.loading = false;
        if (res.code == 0) {
    ...
    deleteItem(item) {
      this.$confirm("删除后不能恢复，您确定吗？", "删除提示", {
        ...
        .then(() => {
          this.loading = true;
          this.$post(
            "admin/common/merch_account/delete",
            { ids: item.id },
            res => {
              this.loading = false;
              if (res.code == 0) {
    ...
    openItem(item, flag) {
      this.loading = true;
      this.$post(
        "admin/common/merch_account/open_or_close",
    ...
    doCommit() {
      ...
      this.$post(
        "admin/common/merch_account/save",
        {
          id: this.formData.id,
