<template>
  <div class="list-wrapper" v-loading="loading">
    <div class="title-bar">
      <h3 class="title">上课地点</h3>
      <div class="buttons" v-if="$hasPermission(102,1002)">
        <el-button type="primary" @click="openForm">添加上课地点</el-button>
      </div>
    </div>
    ...
    <div class="list">
      ...
      >
        ...
        <el-table-column label="名字" prop="name" align="center" width="180"></el-table-column>
        <el-table-column
          label="所属校区"
          column-key="school_name"
          :filters="schools"
        ...
        <el-table-column label="操作" align="center" width="240">
          <template slot-scope="scope">
            <el-button size="mini" v-if="$hasPermission(102,1004)" @click="editItem(scope.row);">编辑</el-button>
            <el-button
              size="mini"
              v-if="$hasPermission(102,1008)"
              type="danger"
            ...
            <el-button
              size="mini"
              type="warning"
              v-if="scope.row.opened && $hasPermission(102,1010)"
              @click="openItem(scope.row, false);"
            >禁用</el-button>
            <el-button
              size="mini"
              type="success"
              v-if="!scope.row.opened && $hasPermission(102,1009)"
...
export default {
  ...
  methods: {
    loadData() {
      this.loading = true;
      this.$get("admin/common/classroom/list", null, res => {
        // console.log(res);
        this.loading = false;
        if (res.code == 0) {
          this.list = res.data;
          ...
          res.data.forEach(obj => {
            if (temp.indexOf(obj.school_name) === -1) {
              temp.push(obj.school_name);
              temp2.push({
                text: obj.school_name,
                value: obj.school_name
              });
            }
          });
          this.schools = temp2;
    ...
    deleteItem(item) {
      this.$confirm("删除后不能恢复，您确定吗？", "删除提示", {
        ...
        .then(() => {
          this.loading = true;
          this.$post("admin/common/classroom/delete", { ids: item.id }, res => {
            this.loading = false;
            if (res.code == 0) {
              this.$message({
                type: "success",
    ...
    openItem(item, flag) {
      this.loading = true;
      this.$post(
        "admin/common/classroom/open_or_close",
